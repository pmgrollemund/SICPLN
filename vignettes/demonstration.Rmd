...
```{r ,eval=TRUE,include = TRUE}
# image de la matrice sigma (precision matrix)
tmp <- solve(spider_pln$res_fisher$model_par$Sigma)
norm <- sqrt(diag(tmp)) %*% t(sqrt(diag(tmp)))
cor_mat <- tmp / norm
precision_mat_sicpln <- plot_precision_matrix(cor_mat, axis_names = c("", "", "", "PLN_Spider precision matrix of Sigma"), limit = NULL)
precision_mat_sicpln
```
