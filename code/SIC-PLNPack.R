...
source(code_path)
#source("/home/gdremesh/Bureau/new_code/SIC-PLNPack.R")
...
# Generation of categorical environmental variables
...
prep_Covar_environement <- data_to_PLN[ , c("vegetation_index","rainfall",
                                            ...
                                            "pluvio_an", "surface", "var_cat", "cat1", "cat2", "cat3")]  # , "var_cat", "cat1", "cat2", "cat3"

# Computing VIF (variance inflation factors)
Res_vif <- calculate_VIF(prep_Covar_environement, 5)

cat("\n\n\t Variable retenue après avoir enlever la colinealité :\n")
print(Res_vif$selected_variables)
...
print(genus2_pln_offset$model_par$B)
...
genus2_sicpln <- SICPLN(formule, offset_column_name = "surface", data = data_to_PLN, control = control)
print(genus2_sicpln$res_fisher$model_par$B)
...
plot_coef_pln <- coef_genus_barre(genus2_pln_offset$model_par$B[-1,], 
                                  axis_names = c("Genus", "Variables", "Coefficients value", "PLN"), -3, 3, data)
plot_coef_sicpln <- coef_genus_barre(genus2_sicpln$res_fisher$model_par$B[-1,],
                                     axis_names = c("Genus species", "Variables", "Coefficients value", "SICPLN"), -3, 3, data)
...
# Utilisation de la fonction "plot_residus_density()" avec les données d'abondance et les valeurs prédites
...
label_order <- paste0("gen",1:ncol(data$Abundance),sep = "")
...
plotcovar_Abund <- plot_abundance_vs_environment(genus2_sicpln$res_fisher$model_par$B, data, output_dir) #, output_dir

combined_density_plot <- plot_combined_density(data$Abundance, genus2_pln_offset$fitted,genus2_sicpln$fitted,
